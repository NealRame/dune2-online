...
const UnitsFlipAxes: Record<number, FlipAxis[]> = {
    1:   [FlipAxis.X],
    2:   [FlipAxis.X],
    3:   [FlipAxis.X],
    6:   [FlipAxis.X],
...
function getFlipAxes(set: ImageSet, index: number): number[] {
    if (set === "units") {
        return (UnitsFlipAxes[index] ?? []) as number[]
    }
    return []
}
...
function ImageCreator(set: ImageSet, palette: Palette)
    ...
    return async (data, index) => {
        ...
        for (const flipAxes of getFlipAxes(set, index)) {
